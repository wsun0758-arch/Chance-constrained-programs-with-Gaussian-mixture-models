% SeDuMi code for marginal risk constrained portfolio optimization

function [solution_opt,objective_value,flag,spend_time]= branch_bound(model,a,B,b,f0,alpha,x0,ub)
global lwz eps;
[At,c,K,lwz0,upz0]=data_input_2(model,a,B,alpha,ub); 
...
ub = [ub*ones(n,1);ones(num,1)];
%Bound on Linear Inequality
rl=1-alpha;
ru=Inf;
%linear matrix
A=[zeros(1,n),model.PComponents];



pars.eps=1e-7; %sedumi acurracy parameter
pars.fid=0; % supress the screen output of sedumi
...
while (Omega_k>0 && abs(v_opt-v_low)>eps0)
    ...
    if initial==0  % the first time to solve sub-problem
     ...
     if isempty(find(lwz<0.5))==1
        c(n-1+n+3:n-1+n+2+2*num) = sparse([-lwz; upz]);      
        for k=1:num
            c(n-1+n+2+2*num+(k-1)*(m+1)+1) =-(f0+a'*model.mu(k,:)')/norminv(full(lwz(k)));
        end
          
        for k=1:num
             At(n-1+n+2+2*num+(k-1)*(m+1)+1,1:n) = sparse([(B*model.mu(k,:)')'/norminv(full(lwz(k)))]);         
        end
        At=sparse(At);
        c=sparse(c);
        [~,x,inf]=sedumi(At,-b,c,K,pars);%%%%%%%%%%%%%%
        y=b'*x;
        if  inf.dinf>0
            branch_feasible=0;
        end
    ...
    if branch_feasible==0    % if the branch-problem is infeasibe
        ...
        if v_opt_i>v_opt || norm(Omega_delta_upz(i_minv,:)-Omega_delta_lwz(i_minv,:))<eps0% cut this branch
            if  Omega_k>i_minv
                Omega_delta_lwz(i_minv,:)=Omega_delta_lwz(Omega_k,:);
                Omega_delta_upz(i_minv,:)=Omega_delta_upz(Omega_k,:);
                Omega_v_low(i_minv)=Omega_v_low(Omega_k);
          ...
          % subdivide rectangle \Delta^i
                ...
                Omega_k=Omega_k-1; 
                [temp,I]=unique([full(Omega_delta_lwz(1:Omega_k,:)),full(Omega_delta_upz(1:Omega_k,:))],'rows');
                temp_Omega_delta_lwz=temp(:,1:num);
                temp_Omega_delta_upz=temp(:,num+1:num*2);
                temp_Omega_v_low=full(Omega_v_low(I));
                Omega_k=length(I);
                
                Omega_delta_lwz=sparse(temp_Omega_delta_lwz);
                Omega_delta_upz=sparse(temp_Omega_delta_upz);
                Omega_v_low=sparse(temp_Omega_v_low);
