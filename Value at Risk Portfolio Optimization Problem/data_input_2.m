% SeDuMi code for VaR constrained portfolio optimization: 
%data input format for SeDuMi

function [At,c,K,lwz0,upz0]=data_input_2(model,a,B,alpha);  
...
num=model.NComponents;
  ...
  c = sparse([1;zeros(n,1);-1+alpha; zeros(num,1);ones(num,1)]);  

   At = sparse([ones(1,n), zeros(1,num)]); %e^T*x = 1
