% Branch and Bound code for VaR risk constrained portfolio optimization

function [solution_opt,objective_value,flag,spend_time]= branch_bound(model,a,B,f0,alpha,x0)
global lwz eps;
eps0=1.0e-5;
...
%data input format for SeDuMi
[At,c,K,lwz0,upz0]=data_input_2(model,a,B,alpha); 
...
while (Omega_k>0 && abs(v_opt-v_low)>eps0)
    ...
    if initial==0  % the first time to solve sub-problem
     ...
     if isempty(find(lwz<0.5))==1
        c(n+3:n+2+2*num) = sparse([-lwz; upz]);      
        for k=1:num
            c(n+2+2*num+(k-1)*(m+1)+1) =-(f0+a'*model.mu(k,:)')/norminv(full(lwz(k)));
        end
          
        for k=1:num
             At(n+2+2*num+(k-1)*(m+1)+1,1:n) = sparse([(B*model.mu(k,:)')'/norminv(full(lwz(k)))]);         
        end
        At=sparse(At);
        c=sparse(c);
